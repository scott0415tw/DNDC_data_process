...
#### heatmap ####
# remove_duplic <- function(df) {
#   cols <- colnames(df)
#   
#   # 去掉尾端的 _數字，例如 soil_soc_1 -> soil_soc
#   prefixes <- sub("_[0-9]+$", "", cols)
#   
#   keep_idx <- !duplicated(prefixes)
#   all_df_unique <- df[, keep_idx, drop = FALSE]
#   
#   return(all_df_unique)
# }
# 
# 
# # 移除不需要的欄位
# all_num <- all_df[ , -c(1,5)] %>%
#   mutate(across(everything(), as.numeric)) %>%
#   select(where(~ !is.na(sd(., na.rm = TRUE)) && sd(., na.rm = TRUE) != 0))%>%
#   remove_duplic()
# 
# # 計算相關矩陣
# cor_mat <- cor(all_num, use = "pairwise.complete.obs")
# 
# # 轉換為長格式
...
#   labs(title = "Correlation Map", fill = "r")
...
p_base <- ggplot(data = all_df) +
  ...
  labs(subtitle = paste0("test name = ", test_batch_name, ", year = ", year, "\n","1st real mean sum: ", round(real_total_1,3),
                      ...
                      "1st residual = ",fst_resid, ", 2nd residual = ",sec_resid),
    y = "Flux",
    x = "Date"
  )
...
#     subtitle = paste("1st residual =",fst_resid, "\n 2nd residual = ",sec_resid),
#     x = "Date", 
#     y = "Top SOC (KgC/Ha)"
#   ) +
#   scale_x_date(breaks = as.Date(date_year), date_labels = "%m/%d") +
